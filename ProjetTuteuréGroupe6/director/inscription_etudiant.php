    ...
    if (empty($nom) || empty($prenom) || empty($matricule) || empty($password) || empty($confirm_password)) {
        ...
        try {
            ...
            if ($stmt_check->fetchColumn() > 0) {
                ...
                $stmt_role = $pdo->prepare("SELECT id FROM roles WHERE nom_role = 'student'");
                $stmt_role->execute();
                $role_id = $stmt_role->fetchColumn();
                if (!$role_id) throw new Exception("Rôle 'student' non trouvé dans la base de données.");
                ...
                $entry = "[$timestamp] Étudiant ajouté: $nom $prenom, Matricule: $matricule" . PHP_EOL;
